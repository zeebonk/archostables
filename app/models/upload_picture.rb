...
class UploadPicture
  ...
  def initialize upload
    ...
    @source_image = MiniMagick::Image.open(upload.path)
    temp = "#{Time.now.strftime("%d%m%Y%H%M%S")}#{Time.now.usec}"
    @filename = "#{temp}.jpg"
  end
   
  # This method creates a small copy of the source image
  def create_small_image
    picture_in_canvas = draw_picture_in_canvas 226, 125
  ...
  def create_medium_image
  	picture_in_canvas = draw_picture_in_canvas 449, 216
